...
typedef struct _XHCI_INPUT_CONTROL_CONTEXT32
{
	/**
...
typedef struct _XHCI_INPUT_CONTROL_CONTEXT64
{
	...
	DWORD RSV1:8;
	DWORD RSV2[8];
} XHCI_INPUT_CONTROL_CONTEXT64;
typedef struct _XHCI_SLOT_CONTEXT32
{
	/**
	 ...
	 * hub, then this field shall contain the number of the downstream facing port of the parent High speed hub109.
	 * For SS and SSP bus instance, if this device is connected through a higher rank hub110 then this
...
typedef struct _XHCI_ENDPOINT_CONTEXT32
{
	/**
	...
	DWORD RSV1:1;
	/**
	 * Error Count (CErr). This field defines a 2-bit down count, which identifies the number of
	...
	DWORD MPSZ:16;
	/**
	 * Bit 0 - Dequeue Cycle State (DCS). This bit identifies the value of the xHC Consumer Cycle State (CCS)
	 ...
	 *
	 * Bit 63:4 - TR Dequeue Pointer. As Input, this field represents the high order bits of the 64-bit base
...
typedef struct _XHCI_ENDPOINT_CONTEXT64
{
	...
	DWORD RSV3[3];
	DWORD RSV4[8];
} XHCI_ENDPOINT_CONTEXT64;
typedef struct _XHCI_INPUT_CONTEXT32
{
	XHCI_INPUT_CONTROL_CONTEXT32 CTRL;
	XHCI_SLOT_CONTEXT32 SLOT;
	XHCI_ENDPOINT_CONTEXT32 ENDP[31];
} XHCI_INPUT_CONTEXT32;
typedef struct _XHCI_INPUT_CONTEXT64
{
	XHCI_INPUT_CONTROL_CONTEXT64 CTRL;
	XHCI_SLOT_CONTEXT64 SLOT;
